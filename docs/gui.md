...
\section{Grafische interfaces met PyQt}

Als je een grafische applicatie schrijft roep je functies aan van het besturingssysteem om vensters, knoppen, menu's e.d. te laten tekenen en te reageren op muisklikken en het toetsenbord. Het lastige daaraan is dat een applicatie voor MacOS heel anders geschreven moet worden dan één voor Linux of Windows. Om die reden zijn er verschillende \emph{cross-platform} bibliotheken ontwikkeld die als het ware tussen het besturingssysteem en je applicatie komen te staan. Je kunt dezelfde applicatie maken voor alle besturingssystemen en de bibliotheek kiest welke functies aangeroepen moeten worden om een venster te tekenen. Het voordeel is duidelijk: je hoeft maar één applicatie te schrijven die overal werkt. Het nadeel is dat je niet écht gebruik kunt maken van alle functies en opties die het besturingssysteem biedt. Hier kiezen we voor de voordelen en gaan we gebruik maken van misschien wel de meest populaire optie: Qt.\footnote{Uitspraak: het Engelse \emph{cute}.} De bibliotheek \texttt{PySide6} is de officiële Pythonbibliotheek.

\begin{attention}
    Maak voor de oefeningen een nieuw conda environment \texttt{IK-test-qt} met:
    \begin{ps1concode}
    PS> conda create -n IK-test-qt python
    PS> conda activate IK-test-qt
    PS> pip install pyside6 pyqtgraph
  \end{ps1concode}
    Selecteer het nieuwe \texttt{IK-test-qt} conda environment in Visual Studio Code en sluit alle `oude' terminals met het \faTrash*-icoon \footnote{of in één keer met \menu{View > Command Palette > Terminal: Kill All Terminals}}.
\end{attention}
...
\begin{pythoncode}
  ...
  if __name__ == "__main__":
      main()  
\end{pythoncode}
Eerst importeren we een paar bibliotheken. Het draait uiteindelijk om de \pythoninline{UserInterface} class. De naam mag je zelf kiezen, zolang je maar aangeeft dat de class een afgeleide is van \pythoninline{QtWidgets.QMainWindow}, het hoofdvenster van je applicatie. In het hoofdgedeelte van het programma (gedefinieerd in de functie \pythoninline{main()}) maak je eerst een instance van \pythoninline{QtWidgets.QApplication} \footnote{Die kun je eventuele command-line arguments meegeven die door Python in \pythoninline{sys.argv} bewaard worden. Meestal zijn die leeg, maar we geven ze gewoon door aan Qt}. Ook maken we een instance van onze eigen class en we roepen de \pythoninline{show()} method aan. Die hebben we niet zelf geprogrammeerd; die zit in de parent class \pythoninline{QMainWindow}. Als laatste roepen we de \pythoninline{exec()} method aan van onze \pythoninline{QApplication} en de uitvoer daarvan (een \emph{exit code}) geven we mee aan de functie \pythoninline{sys.exit()}. Dat betekent dat als het programma afsluit met een foutmelding, dat een foutcode wordt meegegeven aan het besturingssysteem. Iemand anders die een script schrijft kan die code afvangen en daar iets mee doen.

Een aantal elementen uit dit programma (\pythoninline{sys.argv}, \pythoninline{sys.exit()}) zijn strikt genomen niet noodzakelijk, maar wel \emph{good practice}. Ook het schrijven van een \pythoninline{main()} functie is niet strikt noodzakelijk, maar het maakt het wel makkelijk om straks een zogeheten \emph{entry point} te hebben als we weer een applicatie willen schrijven. In de \path{pyproject.toml} geven we dan aan dat we de \pythoninline{main()} functie willen aanroepen. Dat komt later.
...
\subsection{De interface programmeren}
\label{sec:ui-prog}
...
\end{figure}
We doen dat door de class \pythoninline{UserInterface} uit te breiden met widgets uit de \pythoninline{QtWidgets} bibliotheek.

Het definiëren van layouts gebeurt in veruit de meeste opmaaksystemen met rechthoeken (\emph{Engels: boxes}) die op verschillende manieren gestapeld worden -- naast elkaar, boven elkaar, of op een rechthoekig grid bijvoorbeeld. Zulke systemen zijn ook \emph{hiërarchisch}: je stopt boxes in andere boxes. De layout van \figref{fig:screenshot-ui-prog} is als volgt opgebouwd: ieder venster moet één centrale widget hebben (\figref{fig:layout-centerwidget}). Vervolgens geven we aan dat we die verticaal gaan opdelen (\figref{fig:layout-vbox}). In die indeling plaatsen we dan een widget voor tekstinvoer én een widget met een horizontale opdeling (\figref{fig:layout-text-hbox}). Als laatste plaatsen we in die horizontale opdeling twee knoppen (\figref{fig:layout-buttons}). Vergelijk die schematische voorstelling nog een keer met \figref{fig:screenshot-ui-prog}. Het is soms even puzzelen hoe je de layout van een applicatie het beste opbouwt.
...
Het stuk programma om bovenstaande layout op te bouwen geven we hieronder weer.\footnote{Deze code is als \filepath{ex-gui-programming.py} beschikbaar op Canvas.} We bespreken de code regel voor regel \emph{na} de code hieronder.
...
\end{pythoncode*}
Allereerst definiëren we een \verb|__init__()|. Helaas gaat dat niet zomaar. We schrijven namelijk \emph{niet} helemaal zelf een nieuwe class (\pythoninline{class UserInterface}), maar breiden de \pythoninline{QMainWindow}-class uit (\pythoninline{class UserInterface(QtWidgets.QMainWindow)}). Door dat te doen zijn er heel veel methods al voor ons gedefinieerd. Daar hoeven we verder niet over na te denken, onze interface `werkt gewoon'. Het gaat mis als wij zelf nieuwe methods gaan schrijven die dezelfde naam hebben. Stel dat de \emph{parent class} \pythoninline{QMainWindow} een method \pythoninline{click_this_button()} heeft. Als onze class \emph{ook} een method \pythoninline{click_this_button()} heeft, dan zal \emph{die} worden aangeroepen in plaats van de method uit de parent class. Dat is handig als je de parent method wilt vervangen maar niet zo handig als je de parent method wilt \emph{aanvullen}, zoals nodig is bij \verb|__init__()|. Immers, we willen onze eigen class initialiseren, maar we willen ook dat de parent class volledig wordt geïnitialiseerd.

De oplossing is gelukkig vrij eenvoudig: we kunnen de \verb|__init__()| van de parent class gewoon aanroepen en daarna ons eigen ding doen. De Pythonfunctie \pythoninline{super()} verwijst altijd naar de parent class, dus met \pythoninline{super().__init__()} wordt de parent class volledig geïnitialiseerd. Dat is dus het eerste dat we doen in regel 7.

\begin{minopdracht}
    \begin{enumerate}
        \item Breid het script van \opdref{opd:minimal-gui} uit met een \pythoninline{__init__}-method.
...
De knoppen zelf plaatsen we straks in een horizontale layout, dus die voegen we ook toe aan de \pythoninline{vbox}. En we maken de layout compleet (\figref{fig:layout-buttons}) door knoppen toe te voegen aan de \pythoninline{hbox}.
...
\begin{info}
    Widgets zoals knoppen voeg je toe met \pythoninline{addWidget()}. Layouts voeg je toe aan andere layouts met \pythoninline{addLayout()}.
\end{info}

De horizontale layout (voor de knoppen) moeten we expliciet toevoegen aan de verticale layout zodat hij netjes verticaal onder het tekstvenster verschijnt. Merk op dat de verticale layout \pythoninline{vbox} \emph{niet} expliciet wordt toegevoegd (aan de centrale widget). De centrale widget (en \emph{alleen} de centrale widget) krijgt een layout door bij het aanmaken van de layout de parent \pythoninline{central_widget} op te geven, dus: \pythoninline{QtWidgets.QVBoxLayout(central_widget)}. Alle andere widgets en layouts worden expliciet toegevoegd en daarvoor hoef je dus geen parent op te geven.

Als laatste verbinden we de knoppen aan functies. Zodra je op een knop drukt wordt er een zogeheten \emph{signal} afgegeven. Die kun je verbinden met een \emph{slot}. Er zijn ook verschillende soorten signalen. Het drukken op een knop zorgt voor een \emph{clicked signal}, het veranderen van een getal in een keuzevenster geeft een \emph{changed signal}. Wij verbinden één knop direct met een al bestaande method van het tekstvenster \pythoninline{clear()} en de andere knop met een eigen method \pythoninline{add_button_clicked()}. De naam is geheel vrij te kiezen, maar boven de functiedefinitie moet je wel de \pythoninline{@Slot()}-decorator gebruiken. PySide kan dan net wat efficiënter werken.
...
\begin{minopdracht}
    \begin{enumerate}
        ...
        \item Definieer een \pythoninline{add_button_clicked()} functie (regel 30--32) en verbind deze aan de \texttt{Add text}-knop (regel 28).
        \item Test of \filepath{example-gui.py} nog steeds werkt en of de knoppen doen wat je verwacht.
    \end{enumerate}
\end{minopdracht}

Er zijn veel verschillende widgets met eigen methods en signals. Je vindt de lijst hier: \url{https://doc.qt.io/qtforpython/PySide6/QtWidgets/index.html#list-of-classes}. Qt6 zelf bestaat uit C++ code en PySide6 vertaalt alle methods e.d. letterlijk. Vandaar ook de methodnaam \pythoninline{addWidget()} in plaats van \pythoninline{add_widget()}. In C++ en Java is het wel gebruikelijk om functies \texttt{CamelCase} namen te geven als \pythoninline{kijkDitIsEenMooieFunctie()}, maar in Python zijn we \texttt{snake\_case} gewend, als in \pythoninline{kijk_dit_is_een_mooie_functie()}.
...
\begin{minopdracht}
    ...
    \begin{enumerate}
        \item Voeg een derde knop \texttt{Hello, world} toe die de tekst \emph{Hello, world} toevoegt aan het venster.
        \item Zet een hekje voor de \pythoninline{super()}-aanroep of haal de regel weg. Wat gebeurt er als je de code opstart? (Zet de regel weer terug!)
        \item Voeg \emph{onder} de andere knoppen een \texttt{Quit}-knop toe. Als je daar op klikt moet de method \pythoninline{self.close()} aangeroepen worden. Daarmee sluit je het programma af. Denk erom dat als je het \pythoninline{clicked}-signaal verbind met \pythoninline{clicked.connect()} dat je de functie die je meegeeft nog \emph{niet} moet aanroepen maar alleen moet meegeven zodat die \emph{later} kan worden aangeroepen. Concreet betekent dit dat je de haakjes weglaat. Zie ook regel 27 en 28 van bovenstaande code.
    \end{enumerate}
\end{minopdracht}


\subsection{De interface ontwerpen met Qt Designer}
...
\end{pythoncode}
Waarbij de gebruikersinterface geladen wordt uit het bestand en we alleen nog maar de signals aan de slots hoeven te koppelen. In deze code definiëren we niet \pythoninline{self.ui.clear_button} of \pythoninline{self.ui.add_button}; die namen geven we aan de knoppen die we maken in Designer. De namen van alle objecten in Designer zijn daarna beschikbaar in onze code om bijvoorbeeld de signalen te koppelen. Merk op dat we nu niet meer \pythoninline{self.clear_button} gebruiken maar \pythoninline{self.ui.clear_button}. Alle widgets komen op deze manier onder een \pythoninline{.ui}-object te hangen.

\begin{minopdracht}
    \begin{enumerate}
        \item Open Designer en kies bij \menu{templates/forms} voor \pythoninline{MainWindow}. Klik dan op \menu{Create}. Ontwerp de user interface uit \figref{fig:screenshot-ui-prog} en gebruik dezelfde namen voor de widgets als het voorbeeld in \secref{sec:ui-prog}. Dus een \pythoninline{add_button} knop, een \pythoninline{clear_button} knop en een \pythoninline{textedit} tekstveld. Het is niet erg als je venster niet dezelfde grootte heeft. Qt Designer kiest een andere standaardafmeting.
        ...
        \item In een terminal in Visual Studio Code, navigeer naar dezelfde map waarin je je script uit de vorige opdracht hebt staan\footnote{Overleg met elkaar of met de assistent als je niet weet hoe dat moet.} en type in:
              \begin{ps1concode}
            PS> pyside6-uic simple_app.ui -o ui_simple_app.py  
          \end{ps1concode}
...
\section{Een oefening: functieplotter}

Je hebt nu twee manieren gezien om een interface te bouwen: programmeren of Designer gebruiken. Let er wel op dat er dus een subtiel verschil is in het benaderen van de widgets. Je kunt bij zelf programmeren bijvoorbeeld \pythoninline{self.add_button} gebruiken, maar als je Designer gebruikt moet dat \pythoninline{self.ui.add_button} zijn.
...
\subsection{De plotter als script}
...
\begin{info}
    \textbf{Als je je GUI het liefst programmeert}, gebruik dan de volgende regel om een plot widget te krijgen in de \pythoninline{__init__()}:
    \begin{pythoncode}
...
\begin{info}
    ...
    \begin{enumerate}
        \item Voeg aan je interface een \emph{Graphics View} toe;
        \item Klik er op om hem te selecteren en klik daarna op de rechtermuistoets;
        \item Kies voor \emph{Promote To ...};
        \item Bij \emph{Promoted class name} vul je in \texttt{PlotWidget} en bij \emph{Header file} vul je in \texttt{pyqtgraph} (zonder \texttt{.h} aan het eind);
        \item Dan klik je op \emph{Add} en vervolgens op \emph{Promote};
    \end{enumerate}
    Zie \figref{fig:screenshot-promote-widget}. Nu je dit een keer gedaan hebt kun je voortaan op een Graphics View meteen kiezen voor \emph{Promote to \textrightarrow\ PlotWidget} en hoef je niets meer in te typen. Vergeet niet je widget nog even een handige naam te geven, bijvoorbeeld \verb|plot_widget|.
\end{info}

\begin{figure}
    \centering
    \includegraphics[scale=.4]{figures/screenshot-promote-widget.png}
    \caption{Voor het toevoegen van een \texttt{PlotWidget} van PyQtGraph moet je eerste een \texttt{GraphicsView} toevoegen (rode pijl) en die \emph{Promoten} naar een \texttt{PlotWidget} (rood kader).}
...
\end{pythoncode}
Je kunt uiteraard spelen met de instellingen zoals \pythoninline{symbol} en \pythoninline{pen}, of je laat ze weg. Leeg maken kan met \pythoninline{self.plot_widget.clear()}.

\begin{inleveropdracht}[Functieplotter: plot]
    Schrijf een script en ontwerp een (eenvoudige!) grafische interface waarmee je de functie $\sin(x)$ plot in het domein $(-\pi, \pi)$. De `applicatie' hoeft verder niets te kunnen.
\end{inleveropdracht}
...
\begin{bonusopdracht}[Functieplotter: (uitdaging)]
    Gebruik een \pythoninline{QComboBox} om de functie te kunnen kiezen. Je moet hem \emph{leeg} toevoegen aan je interface en vult hem vanuit je programma. Zoek de widget op in de documentatie om uit te zoeken welke functie je moet gebruiken om keuzemogelijkheden toe te voegen en welk signaal je moet koppelen om te zorgen dat de plot opnieuw wordt uitgevoerd als je de functie aanpast. Geef de gebruiker de keuzes $\sin(x)$, $\cos(x)$, $\tan(x)$ en $\exp(x)$.
\end{bonusopdracht}

\begin{bonusopdracht}[Functieplotter (uitdaging)]
    Voeg aan de functiekiezer de functies $x$, $x^2$, $x^3$, en $\frac{1}{x}$ toe. Je kunt daarvoor \emph{lambda functions} gebruiken, maar dat is niet per se nodig.
\end{bonusopdracht}
...
\begin{bonus}
    ...
    \begin{warning}
        Gebruik \emph{nooit} zomaar \pythoninline{eval()} op een string die iemand anders aanlevert. Anders kan iemand met typen in een tekstveld of het inlezen van een tekstbestand je computer wissen bijvoorbeeld, of malware installeren. Als je \pythoninline{eval()} wilt gebruiken, lees dan de sectie \emph{Minimizing the Security Issues of eval()} in \citetitle{eval} \cite{eval}. Maar \emph{veel makkelijker} is om \texttt{asteval} te gebruiken.
    \end{warning}
\end{bonus}


\section{Een grafische interface voor ons experiment}

In het vorige hoofdstuk hebben we een \emph{tekst}interface geschreven voor ons experiment. We gaan nu een \emph{grafische} interface schrijven voor hetzelfde experiment.

We hebben tot nu toe veel moeite gedaan om je code te splitsen volgens het MVC-model: werken in laagjes, goed nadenken over wat waar hoort. Als dat netjes gelukt is kunnen we relatief makkelijk één van die laagjes vervangen. We kunnen de \pythoninline{ArduinoVISADevice} vervangen door een \pythoninline{RaspberryPiDevice} of een \pythoninline{PicoScopeDevice}.\footnote{Je moet dan wel eerst nieuwe controllers schrijven (of krijgen van een collega) om deze nieuwe instrumenten aan te sturen. Maar als je die hebt kun je vrij eenvoudig wisselen.} Ook kunnen we een nieuwe applicatie schrijven voor ons bestaande experiment. We hoeven dan alleen een extra view te schrijven (de interface met de gebruiker) en de rest kunnen we hergebruiken. Misschien dat we hier en daar iets willen aanpassen maar zorg er dan voor dat je oude applicatie nog steeds werkt!
...
\begin{info}
    ...
    \begin{ps1concode}
    PS> poetry add pyside6 pyqtgraph
  \end{ps1concode}
\end{info}
...
\begin{info}
    ...
    \begin{enumerate}
        \item Installeer \emph{Poe the Poet} -- onze \emph{task runner} -- als \emph{development dependency} met:
              \begin{ps1concode}
          PS> poetry add --group dev poethepoet
        \end{ps1concode}
...
\subsection{Van script naar applicatie}
...
\subsection{Bewaren van meetgegevens}

Je kunt na iedere meting de gegevens automatisch wegschrijven naar bestanden zonder dat de gebruiker nog iets kan kiezen, maar je kunt ook gebruik maken van een \texttt{Save}-knop en dialoogvensters. Je kunt de knop koppelen aan een method \pythoninline{save_data()} en daarin de volgende regel opnemen:
\begin{pythoncode}
  filename, _ = QtWidgets.QFileDialog.getSaveFileName(filter="CSV files (*.csv)")
\end{pythoncode}
De functie \pythoninline{getSaveFileName()} opent een dialoogvenster om een bestand op te slaan. Vanwege het filter argument geeft het venster (op sommige besturingssystemen) alleen CSV-bestanden weer. In elk geval geldt op alle besturingssystemen dat als de gebruiker als naam \texttt{metingen} intypt, dat het filterargument ervoor zorgt dat er automatisch \texttt{.csv} achter geplakt wordt.\footnote{Het eerste deel van het argument (\texttt{CSV files}) is vrij te kiezen en geeft alleen informatie aan de gebruiker. Het deel tussen haakjes (\texttt{*.csv}) is het gedeelte dat echt van belang is. Het geeft de extensie die achter alle bestandsnamen geplakt wordt.} De functie geeft twee variabelen terug: \texttt{filename} en (weer) \texttt{filter}. Die laatste kenden we al en gooien we weg met behulp van de weggooivariabele \pythoninline{_}.

Het enige dat het dialoogvenster doet is de gebruiker laten kiezen waar en onder welke naam het bestand moet worden opgeslagen. Je krijgt echt alleen een pad en bestandsnaam terug, de data is \emph{niet} opgeslagen en het bestand is \emph{niet} aangemaakt. De variabele \texttt{filename} is echt niets anders dan een bestandsnaam, bijvoorbeeld: \texttt{/Users/david/LED-rood.csv}. Nadat je die bestandsnaam gekregen hebt moet je dus zelf nog code schrijven zodat het CSV-bestand wordt opgeslagen onder die naam.
...
\subsection{Uitdaging: Menu's, taak- en statusbalken}
...
% \end{enumerate}
% Het lastige hierbij is dat je dus gaat \emph{proberen} om een poort te openen. De kans bestaat dat dat niet lukt, bijvoorbeeld omdat een poort al in gebruik is. Dan krijg je een exception die je af moet vangen met een \pythoninline{try...except}-blok. Je kunt \emph{alle} exceptions afvangen, maar het is mooier om specifieke exceptions af te vangen. Op die manier blijven onverwachte fouten nog steeds een foutmelding geven.\footnote{Dat kan echt heel handig zijn. Menig programmeur zoekt zich suf naar de reden waarom een programma niet werkt om er dan achter te komen dat één of andere onverwachte foutmelding wordt afgevangen zonder dat hij die te zien krijgt. Als iets niet werkt maar je krijgt geen foutmelding dan kun je lang blijven zoeken...} Zulke exceptions kun je vaak in de documentatie vinden maar je kunt het ook blijven proberen op verschillende computers, met of zonder Arduino, om te zien welke exceptions je kunt krijgen tijdens `normaal' gebruik.

% Om je een zoektocht te besparen: wanneer je een device probeert te openen kun je een \texttt{serial.serialutil.SerialException}\footnote{Om de exception te `herkennen' moet je bovenin je code \pythoninline{import serial.serialutil} toevoegen.} krijgen, bijvoorbeeld wanneer het device al in gebruik is. Als je met een device probeert te communiceren en je krijgt geen antwoord, dan krijg je een \texttt{pyvisa.errors.VisaIOError}. Vang in ieder geval die exceptions dus af.
...
% \begin{hogercijfer}
%   \begin{opdracht}
%     Maak een \emph{functie} in de controller module en implementeer bovenstaande procedure. Als een Arduino gevonden wordt, geef dan de volledige poortnaam terug. Als er geen Arduino wordt gevonden, zorg dan voor een zelfgemaakte exception (bijvoorbeeld \texttt{DeviceNotFound}). Zorg ervoor dat je grafische applicatie bij het opstarten zoekt naar een Arduino en die poort verder gebruikt in het programma. De gebruiker hoeft dus geen poort te kiezen; het werkt automatisch!
%   \end{opdracht}
% \end{hogercijfer}

\begin{inleveropdracht}[Pythondaq: selecteer Arduino]
    Maak een keuzemenu (\pythoninline{QComboBox}) zodat je de Arduino kunt selecteren. Je zult in de \verb|__init__()| eerst een lijst van devices moeten maken en die toe moeten voegen aan de widget. Zie ook \tabref{tab:widgets} en de documentatie. Het kan daarbij handig zijn om de device pas te \emph{openen} als je een scan uitvoert en hem te sluiten (schrijf een soort \pythoninline{device.close()}) als de scan is afgelopen. In de controller werk je met een \pythoninline{pyvisa} device en die heeft al een \pythoninline{close()}-method.
\end{inleveropdracht}

\begin{bonusopdracht}[Pythondaq: exception]
    Het is natuurlijk niet zo mooi wanneer je de verkeerde poort kiest en het programma crasht. Vang de exception af en geef een melding (gebruik bijvoorbeeld \pythoninline{QDialog}) dat het device geen Arduino VISA device is. De gebruiker kan daarna een andere poort proberen.
\end{bonusopdracht}


\section{Meerdere dingen tegelijkertijd: threads}
...
De manier waarop besturingssystemen meerdere dingen tegelijk doen is gebaseerd op \emph{processes} en \emph{threads}. Een \emph{process} is, eenvoudig gezegd, een programma. Als je meerdere applicaties opstart zijn dat allemaal processen. Besturingssystemen regelen dat ieder proces een stuk geheugen krijgt en tijd van de processor krijgt toegewezen om zijn werk te doen. Processen zijn mooi gescheiden en kunnen dus eenvoudig naast elkaar draaien. Het wordt iets lastiger als een \emph{proces} meerdere dingen tegelijk wil doen. Dat kan wel, met \emph{threads}. Het besturingssysteem zorgt dat meerdere threads naast elkaar draaien.\footnote{Er is een subtiliteit. In Python draaien threads \emph{niet} tegelijk, maar om de beurt. In de praktijk merk je daar niet veel van: threads worden zó vaak per seconde gewisseld dat het \emph{lijkt} alsof ze tegelijk draaien. Terwijl de ene thread steeds even tijd krijgt voor een meting kan de andere thread steeds even de plot verversen. In het geval van zwaar rekenwerk schiet het alleen niet op. Er draait maar één berekening tegelijkertijd dus threads of niet, het is even snel. Wil je \emph{echt} parallel rekenen, dan moet je kijken naar de \texttt{multiprocessing} module om meerdere processen te starten in plaats van threads.}

Threads geven vaak problemen omdat ze in zekere zin onvoorspelbaar zijn. Je weet niet precies hoe `snel' een thread draait, dus je weet niet zeker wat er in welke volgorde gebeurt. Dit kan leiden tot problemen waarvan de oorzaak maar lastig te vinden is. Google maar eens op \texttt{thread problems in programming}. We moeten dus voorzichtig zijn! Ook is het ombouwen van code zonder threads naar code met threads een klus waar makkelijk iets fout gaat. Het is dus belangrijk dat je \emph{in kleine stapjes} je code aanpast en \emph{vaak test} of het nog werkt.

\begin{info}
    We gaan in het volgende stuk een kleine applicatie ombouwen van `no-threads' naar `threads'. \emph{Voor het gemak} hebben we de view en het model in één bestand gezet. Op Canvas kun je de code downloaden. We raden je ten zeerste aan om het bestand \path{demo-no-threads.py} te downloaden en dan stapje voor stapje de code aan te passen zoals in de handleiding gebeurt. De andere stappen staan alleen op Canvas om bij problemen even te kunnen vergelijken. \emph{Probeer het dus zelf!} Pas daarna ga je aan de slag om je eigen code om te bouwen waarbij de view en het model nu \emph{wel} weer in verschillende bestanden staan. Samenvattend: doorloop dit stuk handleiding \emph{twee keer}. De eerste keer doe je de opdrachten met het demoscript, de tweede keer met je eigen code voor \githubrepo{pythondaq}.
\end{info}
...
\begin{pythoncode*}{linenos}
  ...
  if __name__ == "__main__":
      main()  
\end{pythoncode*}
In regels 15--24 bouwen we een kleine user interface op met een plot widget en een startknop. We koppelen die knop aan de \pythoninline{plot()}-method. In regel 27 maken we ons experiment (het model) aan en bewaren die. In regels 29--33 maken we de plot schoon, voeren we een scan uit en plotten het resultaat. Regels 37--44 vormen ons experiment. Eerst wordt een rij $x$-waardes klaargezet en dan, in een loop, wordt punt voor punt de sinus uitgerekend en toegevoegd aan een lijst met $y$-waardes. De \pythoninline{time.sleep(.1)} wacht steeds \qty{0.1}{\second} en zorgt hiermee voor de simulatie van \emph{trage} metingen. En inderdaad, als we deze code draaien dan moeten we zo'n vijf seconden wachten voordat de plot verschijnt.

In de volgende opdrachten gaan we de code stap voor stap ombouwen naar threads. Als we daarmee klaar zijn worden de metingen gedaan binnen de \pythoninline{scan()}-method van de \pythoninline{Experiment()}-class en verversen we ondertussen af en toe de plot. De \pythoninline{plot()}-method van onze user interface wordt regelmatig aangeroepen terwijl de meting nog loopt en moet dus de hele tijd de huidige metingen uit kunnen lezen. Dat kan, als de metingen worden bewaard in \emph{instance attributes}.\footnote{Variabelen die we in een class definiëren door ze aan te maken met \pythoninline{self.} ervoor zijn \emph{instance attributes}.}


\subsection{Stap 1: de meetgegevens altijd beschikbaar maken}

We maken in de \pythoninline{scan()}-method lege lijsten \pythoninline{self.x} en \pythoninline{self.y}. Hier komen de meetgegevens in en die staan dus los van de lijst met $x$-waardes die je klaarzet. Met andere woorden: de variabele \pythoninline{x} is \emph{niet hetzelfde} als de variabele \pythoninline{self.x}:
\begin{pythoncode}
  # model
  class Experiment:
      ...
...
\begin{inleveropdracht}[Pythondaq: threads I]
    Pas je eigen code aan zodat de meetgegevens altijd beschikbaar zijn. \emph{Test je code,} je applicatie moet nog steeds werken.
\end{inleveropdracht}


\subsection{Stap 2: plot de meetgegevens vanuit het experiment}

Nu we de meetgegevens bewaren als instance attributes van de \pythoninline{Experiment}-class kunnen we die ook plotten. We geven ze nog steeds terug als return value vanuit de \pythoninline{scan()}-method voor `ouderwetse' code,\footnote{Door een beetje ons best te doen kunnen we ervoor zorgen dat zowel de command-line interface als de grafische interface allebei gebruikt kunnen worden.} maar wij gaan nu de `nieuwerwetse' instance attributes gebruiken:
\begin{pythoncode}
  # view
  class UserInterface(QtWidgets.QMainWindow):
  ...
...
\begin{inleveropdracht}[Pythondaq: threads II]
    Pas je eigen code aan zodat je instance attributes gebruikt voor het plotten. \emph{Test je code,} het moet nog steeds werken als vanouds.
\end{inleveropdracht}


\subsection{Stap 3: threads}

We gaan nu met threads werken. Je importeert daarvoor de \pythoninline{threading} module en maakt voor iedere thread een \pythoninline{threading.Thread()} instance. Deze heeft twee belangrijke parameters: \pythoninline{target} waarmee je de functie (of method) aangeeft die in de thread moet worden uitgevoerd, en \pythoninline{args} waarmee je argumenten meegeeft voor die functie of method. We maken een \emph{nieuwe} method \pythoninline{start_scan()} waarmee we een nieuwe thread starten om een scan uit te voeren. We doen dit als volgt:
\begin{pythoncode}
  import threading
...
In plaats van dat onze plotfunctie de \pythoninline{scan()}-method aanroept, moeten we nu de \pythoninline{start_scan()}-method aanroepen. Maar: die method start een scan en sluit meteen af, terwijl de daadwerkelijke meting op de achtergrond wordt uitgevoerd. De plotfunctie moet -- in deze stap nog even -- wachten tot de scan klaar is. Er is een manier om op een thread te wachten. Je moet daartoe de \pythoninline{join()} method van de thread aanroepen. In bovenstaande code hebben we de thread bewaard in de variabele \pythoninline{_scan_thread}, dus hij is voor ons beschikbaar:
\begin{pythoncode}
  # view
  class UserInterface(QtWidgets.QMainWindow):
      ...
...
\begin{inleveropdracht}[Pythondaq: threads III]
    Pas je eigen code aan zodat je een thread opstart om de scan op de achtergrond uit te voeren. Roep in je plotfunctie de goede method aan en wacht tot de thread klaar is. \emph{Test je code.} Wederom moet het werken als vanouds. Kijk ook eens wat er gebeurt als je \emph{niet} wacht tot de metingen klaar zijn door de regel \pythoninline{self.experiment._scan_thread.join()} uit te commentariëren (hekje ervoor). Niet vergeten het hekje weer weg te halen.
\end{inleveropdracht}


\subsection{Stap 4: plotten op de achtergrond}

We zijn er nu bijna. We gebruiken threads om de metingen op de achtergrond uit te voeren maar we wachten nog steeds tot de metingen klaar zijn voordat we -- eenmalig -- de grafiek plotten. In deze laatste stap doen we dat niet meer. Als je straks op de startknop drukt dan start de meting op de achtergrond. Ondertussen wordt er regelmatig geplot. Je ziet dan tijdens de metingen de plot opbouwen. We doen dat door het scannen en plotten van elkaar los te koppelen -- niet meer samen in één functie -- en door met een \pythoninline{QTimer} de plotfunctie periodiek aan te roepen. Kijk de code goed door. De \pythoninline{...} geven aan waar code onveranderd is gebleven en de \pythoninline{plot()}-method is volledig vervangen en weer \emph{bijna} terug in de oorspronkelijke vorm. Als je twijfelt, kijk dan naar de volledige code op Canvas.
\begin{pythoncode}
...
\begin{inleveropdracht}[Pythondaq: threads IV]
    Pas je eigen code op dezelfde manier aan zodat de metingen op de achergrond worden uitgevoerd terwijl je de plot ziet opbouwen. Je code werkt nu \emph{niet} als vanouds, maar \emph{voelt} veel sneller!
\end{inleveropdracht}


\subsection{Uitdaging: Stap 5: puntjes op de `i': \emph{events}}

Wanneer je op de startknop drukt, even wacht en dan wéér op de startknop drukt, dan kun je zien dat er \emph{twee} metingen tegelijk worden uitgevoerd op de achtergrond. Dat wil je voorkomen. Ook is het wel aardig om metingen tussentijds te kunnen stoppen. Dat is vooral handig als je merkt dat een meting veel te lang gaat duren. Verder is het ook nog zo dat we er nu met onze timer voor gezorgd hebben dat de plotfunctie meerdere keren per seconde wordt uitgevoerd -- of er nu een meting loopt of niet.

Je kunt dit oplossen met \pythoninline{threading.Event()} objecten. Dit zijn objecten met \pythoninline{set()}, \pythoninline{clear()} en \pythoninline{wait()} methods om gebeurtenissen aan te geven of er op te wachten. Zo kun je een event \pythoninline{is_scanning} aanmaken die je \pythoninline{set()} zodra een meting begint en \pythoninline{clear()} zodra de meting is afgelopen. Je controleert bij de start van de meting dan bijvoorbeeld eerst of de meting al loopt met \pythoninline{is_scanning.is_set()} en start alleen een meting als dat nog niet zo is.

Ook kun je in de grafische interface na het starten van een meting de startknop onbeschikbaar maken met \pythoninline{start_button.setEnabled(False)} en weer beschikbaar maken met \pythoninline{start_button.setEnabled(True)}. De knop wordt dan tussendoor grijs. Dat kan handig zijn om duidelijk te maken dat een meting al loopt en dat je niet nogmaals op de startknop kunt drukken.
