\chapter{Versiebeheer met GitHub}

\section{Versiebeheer}

Zodra je scripts wat ingewikkelder worden begin je tegen hele praktische problemen aan te lopen. Het werkt \emph{nu}, maar je wilt een flinke aanpassing gaan doen. Werkt het straks nog wel? Hoe ingewikkelder het script, hoe ingewikkelder de wijzigingen en hoe minder het vertrouwen dat het in één keer gaat lukken. Misschien heb je wel eens de ervaring gehad dat het maar niet wil werken en dat je niet goed weet wat je precies had veranderd ten opzichte van toen het nog \emph{wel} werkte. Veel mensen hebben de neiging om naast een \filepath{script.py} een \filepath{script-v1.py}, \filepath{script-v2.py}, enz. aan te maken. Soms zelfs een \filepath{script-eindversie.py} en met wat pech dan toch nog een \filepath{script-eindversie-definitief.py}. Niet heel fijn. Je ziet dan nog steeds niet goed wat er veranderd is (dat blijft naast elkaar leggen en zoeken) en je map loopt vol met overbodige scripts. Dit kan beter\ldots\ met versiebeheer!

Versiebeheer (Engels: \emph{version control}) stelt je in staat om af en toe een momentopname te maken van al je bestanden in een bepaalde map, inclusief alle submappen. Dit doe je niet na iedere regel code, maar bijvoorbeeld wel als je een stukje code af hebt en getest hebt dat het werkt. Zo'n momentopname heet een \emph{commit}. Hoe vaak je commit is aan jou; maar wacht niet te lang -- dan is het geen versiebeheer meer.
...
Git werkt in principe via de command-line. Je geeft opdrachten in de map waar je broncode staat: toevoegen van wijzigingen aan de \emph{staging area}, bekijken van de meest recente wijzigingen, committen van je code, teruggaan en werken met oudere versies, aanmaken van \emph{branches},\footnote{Een branch is een splitsing in je versiegeschiedenis. Je kunt het gebruiken om over een langere tijd een grote wijziging uit te testen terwijl je af en toe heen en weer springt tussen je main branch en de nieuwe branch. Commits in de nieuwe branch blijven gescheiden. Later kun je ervoor kiezen om de wijzigingen in de nieuwe branch te \emph{mergen} met je main branch, maar dat hoeft niet.} je wijzigingen uploaden naar internet, enz. Het geheel van map met broncode en versiegeschiedenis wordt een \emph{repository} genoemd.
...
\begin{minopdracht}
    Ga naar \url{https://github.com/} en klik rechtsboven op \texttt{Sign Up}. Maak een account aan onder je \emph{privé-emailadres}. Op deze manier blijf je toegang houden tot je account ook nadat je afgestudeerd bent.
\end{minopdracht}
...
\begin{minopdracht}
    ...
    \begin{enumerate}
        ...
        \item Je kunt de repository niet toevoegen omdat de map weliswaar bestaat, maar nog geen bestaande repository is. Er verschijnt een waarschuwing met een stukje kleine blauwe tekst. Klik op \texttt{create a repository}.
        \item Vink \texttt{Initialize this repository with a README} aan.
        \item Kies bij \texttt{Git ignore} voor `Python'.\footnote{De Git Ignore zorgt ervoor dat allerlei hulpbestanden van Python niet bewaard
                  worden als commit. Alleen je eigen code wordt dan bewaard}
        \item En bevestig dan met de blauwe knop \texttt{Create Repository}.
        \item Als je nu op de history klikt dan zie je dat er een \texttt{Initial commit} is met wat \texttt{git}-bestanden en de Pythonscripts die je in de map hebt gezet. Vanaf nu staat \folderpath{Oefenopdrachten} in versiebeheer en houdt Git je wijzigingen bij.
    \end{enumerate}
...
\begin{minopdracht}
    ...
    \begin{enumerate}
        ...
        \item Links verschijnt een blauw bolletje bij \texttt{Source Control} \faCodeBranch\ die laat weten dat er wijzigingen zijn ten opzichte van de vorige commit. Klik op \texttt{Source Control}.
        \item Onder \texttt{Changes} staat een lijst met bestanden waar wijzigingen in aan zijn gebracht. Kies welke bestanden je wilt committen door rechts op het +je te klikken. Deze bestanden komen nu op het podium te staan onder \texttt{Staged Changes}. Je kunt ook alle bestanden in een keer op het podium zetten door naast het kopje \texttt{Changes} op het +je te klikken.
        \item Schrijf een nuttige `commit message'. Dus niet: `\opdref{opd:commit}', maar: `feat: search for port name to open communication with device'.\footnote{Je kunt je commit message opdelen in een titel (of summary) en een beschrijving. In Visual Studio Code, doe je dit door een witregel toe te voegen tussen de titel en de beschrijving.}
...
\begin{inleveropdracht}[Pythondaq: github]
    ...
    \begin{enumerate}
        \item Maak een map \folderpath{pythondaq} en kopieer daarin alle python-bestanden die je gemaakt hebt voor het diode-experiment. Let er op dat je deze map \emph{niet} in een andere repository aanmaakt, maar daarbuiten. Overleg eventueel over een handige plek.
        \item Maak van de map \folderpath{pythondaq} een repository.
        \item Gooi oude bestanden weg en zorg dat je 3 bestanden over houdt: een model \filepath{diode\_experiment.py}, een view \filepath{view.py} en een controller \filepath{arduino\_device.py}.
        \item Commit!
...
\begin{bonusopdracht}
    ...
    \begin{enumerate}
        ...
        \item Je komt terecht op de hoofdpagina, hier zie je een mappenstructuur met een aantal bestanden. Rechts daarvan staat een korte beschrijving onder het kopje \texttt{About}. Een uitgebreidere beschrijving vind je als je naar beneden scrollt onder \texttt{Readme}.
        \item Linksboven zie je een aantal tabbladen (code, issues, pull requests, ...), het tabblad \texttt{code} is de hoofdpagina met de mappenstructuur. Navigeer door de mappen, wat staat er op regel 15 van \filepath{plot\_tab.py}?
        \item Ga terug naar de hoofdpagina, kijk onder het groen kopje met \texttt{code}. Hoeveel commits zijn er gemaakt? Klik op commits en daarna op een commit-message. Hoeveel regels zijn er weggehaald of bijgekomen?
        \item Je kan per bestand bekijken wanneer die is aangepast en wat er is aangepast met de history knop. Ga naar het bestand \filepath{pyproject.toml} en klik rechtsboven op \texttt{History}. Wat is er aangepast in \filepath{pyproject.toml} bij de commit `Release v1.5.1'? Je ziet ook welke bestanden nog meer zijn gewijzigd in deze commit, welk bestand is nog meer gewijzigd bij `Release v1.5.1'?
        \item Ga terug naar de hoofdpagina. Welke versie van Tailor is als laatste gereleased?
        \item Je kent het misschien wel, dat je een app gebruikt maar dat het niet helemaal goed werkt (bug), of je hebt een idee hoe het nog beter kan worden (enhancement). Daarvoor is op GitHub het tabblad \texttt{Issues}. Hoeveel bugs zijn er gerapporteerd? En hoeveel enhancements?
        \item Als het jou nu gelukt is om een bug te fixen, of je hebt een super handige feature ontworpen, dan kan je de eigenaren van de repository vragen om jouw code te implementeren door een pull request te sturen. Ga naar het tabblad \texttt{Pull requests}, klik op \texttt{Closed} en bekijk welke pull requests zijn geïmplementeerd.
        \item Het meest rechter tabblad \texttt{Insights} geeft je, tegen alle verwachtingen in, inzicht. Je kan zien door hoeveel mensen er aan gewerkt wordt. Kijk bij \texttt{Code frequency}, in welke periode is er het meest aan de code veranderd?
        ...
        \item Dan tot slot die ene, meest in het oogspringende groene \texttt{code} knop. Met die knop kan je de repository als zip-bestand downloaden of openen met GitHub desktop.
    \end{enumerate}
\end{bonusopdracht}
...
\begin{bonusopdracht}
    ...
    \begin{enumerate}
        \item Zoek de repository op GitHub op (\texttt{lmfit/lmfit-py})
        ...
        \item Open \path{examples/README.txt}. Verander in de eerste paragraaf \texttt{Below are examples} in \texttt{Below are different examples} en sla het bestand op.
        \item Schakel naar de GitHub Desktop applicatie en bekijk de wijziging.
        \item Linksonder kun je een korte beschrijving van je wijziging intypen en druk dan op de blauwe \texttt{Commit}-knop.
        \item Schakel, rechtsboven, naar \texttt{History}. Bovenaan staat jouw wijziging. Daaronder kun je alle wijzigingen van anderen bekijken.
    \end{enumerate}
\end{bonusopdracht}

Aangezien je geen schrijfrechten hebt voor LMfit kun je niet kiezen voor \texttt{Push origin} -- de knop die rechtsboven verschijnt. Met die knop `duw' je je wijzigingen naar GitHub zodat iedereen ze kan zien. Dat is mooi, maar je mag niet zomaar de repository van iemand anders wijzigen.
...
\begin{bonusopdracht}
    ...
    \begin{enumerate}
        \item Publiceer een repository op Github -- bijvoorbeeld je oefenopdrachten of je \texttt{pythondaq}-repository. Verander iets in je code, commit en kijk hoe je dat naar GitHub kunt Pushen.
...
\begin{bonusopdracht}
    ...
    \begin{enumerate}
        ...
        \item Gebruik de terminal in Visual Studio Code en bekijk de commit geschiedenis met het commando \texttt{git log}. Scroll door de commit messages met spatie.
