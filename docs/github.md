...
!!! opdracht-basis "Repository toevoegen"
    ...
    1. Maak &mdash; bijvoorbeeld vanuit Visual Studio Code &mdash; een map {{folder}}`Oefenopdrachten` en zet daarin alle python-bestandjes die je hebt gemaakt om te oefenen zoals de opdrachten [_Pyvisa in pythonscript_](communicatie.md#opd:test_arduino) en [_KnipperLED_](communicatie.md#opd:flashingLED).
