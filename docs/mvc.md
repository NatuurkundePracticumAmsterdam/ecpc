...
\section{MVC en het gebruik van packages}

MVC staat voor \emph{Model-View-Controller} en is een belangrijk, maar wat diffuus concept in software engineering en is vooral van toepassing op gebruikersinterfaces. Het belangrijkste idee is dat een programma zoveel mogelijk wordt opgesplitst in onderdelen. Het \emph{model} bevat de onderliggende data en concepten van het programma (een database, meetgegevens, berekeningen, etc.); de \emph{controller} praat met de fysieke omgeving en reageert bijvoorbeeld op invoer van een gebruiker en past het model aan; de \emph{view} is een weergave van de data uit het model en vormt de gebruikersinterface zelf. Vaak praten alle onderdelen met elkaar, maar een gelaagd model is makkelijker te overzien en dus eenvoudiger te programmeren. In het geval van een natuurkunde-experiment is dit vaak mogelijk. Daarmee krijgt MVC bij ons een andere betekenis dan bijvoorbeeld bij het bouwen van websites. Het gelaagd MVC-model dat wij gaan gebruiken is weergegeven in \figref{fig:mvc-model}.
\begin{figure}
    \centering
    \includestandalone{figures/mvc-model}
    \caption{Een gelaagd \emph{Model-View-Controller} patroon. De \emph{controllers} communiceren met de apparatuur; het \emph{model} bevat de meetgegevens, berekeningen en de opzet van het experiment; de \emph{view} zorgt voor een gebruikersinterface met weergave van de data.}
    \label{fig:mvc-model}
\end{figure}

Het scheiden van je programma in deze lagen kan enorm helpen om ervoor te zorgen dat je geen \emph{spaghetticode} schrijft -- ongestructureerde en moeilijk te begrijpen code. Wanneer het drukken op een knop in de code van de grafische omgeving direct commando's stuurt naar de Arduino of dat de code voor het doen van een enkele meting meteen de $x$-as van een grafiek aanpast, sla je lagen over in ons model en knoop je delen van het programma aan elkaar die niet direct iets met elkaar te maken hebben. De knop moet een meting starten, ja, maar \emph{hoe} dat precies moet is niet de taak van de gebruikersinterface. En de meting zelf moet zich niet bemoeien met welke grafiek er precies getekend wordt. Je zult merken dat het heel lastig wordt om overzicht te houden en later aanpassingen te doen als je alles door elkaar laat lopen. Je zult dan door je hele code moeten zoeken als je óf de aansturing van de Arduino, óf de grafische interface wilt aanpassen. En dus gaan we alles netjes structureren.

De verschillende onderdelen in \figref{fig:mvc-model} kunnen we voor ons experiment als volgt beschrijven:
\begin{description}
    \item[View] Het `startpunt' van je applicatie. Geeft de opdracht om een meting te starten en geeft na afloop de resultaten van de meting weer op het scherm.
...
Het opsplitsen van je programma \emph{hoeft niet in één keer!} Dit kan stapsgewijs. Je kunt starten met een eenvoudig script -- zoals we hierboven gedaan hebben -- en dat langzaam uitbreiden. Je begint klein, verdeelt je code in lagen en bouwt vervolgens verder.


\section{Gebruik van classes}

Voor een snelle meting is het script dat je geschreven hebt bij \opdref{opd:quickndirty-meting} en \opdref{opd:quickndirty-csv} prima! Maar als de meetapparatuur ingewikkelder wordt (meer verschillende commando's) of je wilt meer aanpassingen doen, dan is het wel lastig dat je op allerlei plekken de commando's opnieuw moet programmeren -- en eerst moet opzoeken. Als je een nieuw script schrijft moet je opnieuw goed opletten dat je de goede \emph{terminator characters} gebruikt, etc. Het is wat werk, maar toch heel handig, om je code op te splitsen en een \emph{class} te schrijven.
...
Lange programma's worden vaak opgedeeld in functies en dat maakt het al een stuk makkelijker omdat functies hun eigen ruimte voor variabelen hebben. In het volgende geval wordt de variabele \pythoninline{first_name} \emph{niet} overschreven:
...
De code werkt nog op dezelfde manier, fijn! Het is alleen wel lastig dat je de hele tijd een variabele \pythoninline{cart} moet meegeven aan de functies. Je bedenkt nog veel meer functies om de levertijd te controleren, om de bestelling op te splitsen in verschillende bezorgmomenten, om de bestelling af te rekenen, etc. Elke keer moet je dezelfde variabele blijven meegeven. Een oplossing is het gebruiken van een class. Je begint met een regel \pythoninline{class Cart:} en daaronder plaats je de functies die je geschreven hebt -- de zogeheten \emph{methods} van de class. Iedere method krijgt automatisch een parameter \pythoninline{self} mee die verwijst naar de gedeelde geheugenopslag. Zo kun je een list \pythoninline{self.contents} definiëren waarin we de producten in de bestelling bewaren. Opstarttaken kun je onderbrengen in de speciale method \pythoninline{__init__()}. Als volgt:
\begin{pythoncode}
    class Cart:
        def __init__(self):
            self.contents = []
...
Wanneer we deze class gaan gebruiken hoeven we de parameter \pythoninline{self} \emph{niet} mee te geven, dat gaat automatisch. Wel moeten we de class eerst `klaarzetten' voor gebruik door hem aan te roepen:
\begin{pythoncode}
    cart = Cart()
    ...
    # The Hunger Games by Suzanne Collins
\end{pythoncode}
In bovenstaande code hebben we de \emph{class} \pythoninline{Cart} en de \emph{class instance} \pythoninline{cart}. Je kunt meerdere instances hebben van dezelfde class, bijvoorbeeld wanneer meerdere klanten in de webshop tegelijkertijd winkelmandjes vullen:
\begin{pythoncode}
    class Cart:
        ...
...
\begin{pythoncode}
    ...
    ...
\end{pythoncode}
Op deze manier kun je code ook makkelijker delen en verspreiden. Zodra je een class definieert zal Visual Studio Code tijdens het programmeren je code automatisch aanvullen. Zodra je type \pythoninline{cart.add} hoef je alleen maar op \keys{\tab} te drukken en VS Code vult de rest aan.
...
\begin{minopdracht}[class Particle]
    Maak een class \pythoninline{Particle} die de naam van het deeltje en de spin van het deeltje bewaard. Een method \pythoninline{is_up_or_down()} vertelt je of het deeltje spin omhoog (positief) of spin omlaag (negatief) heeft. Maak nog een method \pythoninline{flip()} die de spin van het deeltje omkeert. De volgende code zou moeten werken:
    \begin{pythoncode}
...
\begin{inleveropdracht}[class ElectronicLoadMeasurements]
    % \label{opd:class}
    Schrijf een class \pythoninline{ElectronicLoadMeasurements} waarmee je spanningsmetingen aan een weerstand (\emph{load}) kunt bewaren. De class moet voldoen aan deze eisen:
    \begin{enumerate}
        \item Een method \pythoninline{add_measurement(R, U)} waarmee je een gekozen weerstandswaarde en een gemeten spanning kunt toevoegen aan de lijst van metingen.
        \item Een method \pythoninline{get_loads()} om de gekozen weerstanden in één keer terug te vragen.
        \item Een method \pythoninline{get_voltages()} om de gemeten spanningen in één keer terug te vragen.
        \item Een method \pythoninline{get_currents()} om een lijst stroomsterktes op te vragen, berekend op basis van de metingen.
        \item Een method \pythoninline{get_powers()} om een lijst vermogens op te vragen, berekend op basis van de metingen.
        \item Een method \pythoninline{clear()} waarmee je alle metingen in één keer kunt wissen.
    \end{enumerate}
...
\section{Implementeren van MVC}

\begin{inleveropdracht}[Pythondaq: ArduinoVISADevice]
    \label{opd:meting-class}
    Pak je script van \opdref{opd:quickndirty-csv} erbij en schrijf bovenaan -- maar \emph{onder} de \pythoninline{import}-statements -- een class \pythoninline{ArduinoVISADevice}. Schrijf methods voor die class zodat onderstaande code minimaal zou moeten kunnen runnen:
    \begin{pythoncode}
    ...
    \end{pythoncode}
    Bekijk bovenstaande code goed. Wat doet iedere regel met de Arduino? Overleg met elkaar of met je assistent. Wat is het verschil tussen \pythoninline{get_output_value()} en \pythoninline{get_input_value()}?

    Pas je script -- en vooral ook de class! -- aan zodat hij precies hetzelfde doet als bij \opdref{opd:quickndirty-meting}. Gebruik bovenstaande code dus \emph{alleen} als voorbeeld van welke methods je moet schrijven. Zorg ervoor dat alle firmwarecommando's ondergebracht zijn in de class en dat in je `experiment'-code alleen maar aanroepen naar de class zitten.
\end{inleveropdracht}
...
%   Schrijf mooie docstrings bij de class. Schrijf een docstring voor de class zelf (dus boven de \pythoninline{__init__()}-methode) en voor alle methodes. Gebruik bijvoorbeeld de \citetitle{google_style_guide} \cite{google_style_guide} (sectie 3.8) voor voorbeelden van docstrings. Test de docstring door in je script de regel
%   \begin{pythoncode}
...
De class die we gemaakt hebben voor de aansturing van de Arduino valt in de categorie \emph{controller}. Het laatste stuk waar de plot gemaakt wordt is dus eigenlijk een \emph{view} en de rest van de code -- waar de metingen worden uitgevoerd en de stroomsterkte $I$ wordt berekend -- is een \emph{model}. We gaan de code nog wat verder opsplitsen om dat duidelijk te maken én onderbrengen in verschillende bestanden -- dat is uiteindelijk beter voor het overzicht.

\begin{inleveropdracht}[Pythondaq: Controller afsplitsen]
    Pas het script aan uit opdracht \opdref{opd:meting-class}. Knip de class uit het bestand en plak die in een nieuw bestand \pythoninline{arduino_device.py}. Knip en plak \emph{ook} de functie \pythoninline{list_devices()}, zodat alle \pythoninline{pyvisa}-code netjes in één bestand zit. Je vervangt de functie en de class in het oorspronkelijke script door dit import statement:
    \begin{pythoncode}
...
\begin{inleveropdracht}[Pythondaq: Model afsplitsen]
    ...
    \begin{enumerate}
        ...
        \item Maak een class met (bijvoorbeeld) de naam \pythoninline{DiodeExperiment} en een method \pythoninline{scan()} die de meting met de for-loop uitvoert. Controleer dat het werkt.
        \item Volgens het schema praat alleen het model met de controller. De class \pythoninline{DiodeExperiment} -- het model -- is dus degene die de class \pythoninline{ArduinoVISADevice} -- de controller -- moet aanroepen en bewaren. Hoe doe je dat netjes? Overleg met elkaar.
        \item Het kan (later) handig zijn om niet altijd te scannen tussen 0 en 1023 maar een ander bereik te kiezen. Pas de \pythoninline{scan()} method aan zodat deze \texttt{start}- en \texttt{stop}-parameters accepteert.
        \item Knip de class eruit en plaats die in het bestand \pythoninline{diode_experiment.py} en gebruik weer een import-statement. Haal import-statements die je niet meer nodig hebt weg.
