...
In de handleiding staan verschillende opgaven. De meeste zijn bedoeld als oefening, maar sommige moet je inleveren voor feedback en een beoordeling. Schrijf je code in het Engels, zo ben je voorbereid op het werken in een internationale onderzoeksgroep.

!!! info
    In sommige programmeercursussen is het de bedoeling dat je een bepaald algoritme _zelf_ schrijft. Je moet bijvoorbeeld een loop schrijven om een reeks berekeningen uit te voeren en mag dan niet gebruik maken van de NumPy-bibliotheek om dat in één regel te doen. Je kunt je voorstellen dat je _juist_ gebruik wilt maken van bibliotheken die het leven een stuk makkelijker maken als je straks in een lab werkt. Trek dus alles uit de kast. Kijk eens in de _Python Standard Library_,[@python-standard-library] de _Python Package Index_[@pypi] of _Awesome Python_[@awesome-python].
...
!!! opdracht-basis "Basisopdracht"
    Deze opgaven helpen je om het niveau te behalen wat van je verwacht wordt. Ze worden niet beoordeeld.
...
!!! opdracht-meer "Meer leren"
    Dit zijn verdiepende en verbredende opgaven om je te kunnen ontwikkelen tot een goed programmeur en een waardevolle aanwinst voor een onderzoeksgroep. Je kunt er geen extra punten mee verdienen wanneer je deze technieken toepast in je inleveropdrachten, maar het is wel een goede oefening. Doe deze opgaven _alleen_ als je klaar bent met de rest.
